...
#1. Parse arguments for input file and output file.
#	1a. Mode = unpack -> accepts file or directory and converts BIC to JSON
#	input = newStringStream(readFile(inputfile)
#	state = input.readGffRoot(false)
#	openfilestream(outputpath, fmWrite)
#	j = state.toJson
#	postProcessJson(j)
#	output.write j.pretty or $j
#	output.write ("\n")

#	1b. Mode = maintenance -> accepts commands to alter JSON
#	nim_bic_editor.nim

#	1c. Mode = repack -> accepts file or directory and converts JSON to BIC
#	input = newStringStream(readfijle(inputfile))
#	state = input.parseJson(inputfile).gffRootFromJson()
#	openfilestream(outputpath, fmWrite)
#	output.write(state)